classdef ProbabilityAnalysisM4Dev < handle
    ...
    methods
        function obj = ProbabilityAnalysisM4Dev(extender)
            % Init
            if nargin < 1
                obj.ext = ProbabilityAnalysisM4Extender;
            else
                if ischar(extender) || isStringScalar(extender)
                    obj.ext = ProbabilityAnalysisM4Extender(extender);
                else
                    obj.ext = extender;
                end
            end
        ...
        function check(obj)
            ...
            upd = ProbabilityAnalysisM4Updater(obj.ext);
            upd.fetch();
